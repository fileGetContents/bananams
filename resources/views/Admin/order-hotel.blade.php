            ...
            @foreach($hotel as $value)
                <tr class="text-c">
                    ...
                    <td><u style="cursor:pointer" class="text-primary"
                           onclick="member_show('张三','member-show','10001','360','400')">{{$value->order_name}}</u></td>
                    <td>{{$value->order_telephone}}</td>
                    <td>{{$value->order_room_num}}</td>
                    <td>{{$value->order_remark}}</td>
                    <td class="text-l">{{$value->hotel_name}}</td>
                    ...
                    <td class="td-status">
                        ...
                        @elseif($value->order_tag == 20)
                            <span class="label label-success radius">已接单</span>
                        @elseif($value->order_tag)
                            <span class="label label-success radius">拒绝单</span>
                        @endif
                    </td>
                    <td class="td-manage">
...
<script type="text/javascript">
    ...
    function member_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            $.ajax({
                type: "post",
                data: {'table': 'hotel_order', 'file': 'order_id', 'where': id},
                dataType: "json",
                url: "{{URL('all/del')}}",
