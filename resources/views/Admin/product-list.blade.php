...
<div>
    <div class="pd-20">
        <div class="cl pd-5 bg-1 bk-gray mt-20">
            <span class="l">
               <a class="btn btn-primary radius" onclick="product_add('添加产品','product-add')" href="javascript:;">
                ...
                @foreach($good as $value)
                    <tr class="text-c va-m">
                        ...
                        <td><a onClick="product_show('','product-show','10001')" href="javascript:;">
                                <img width="60" class="product-thumb" src="{{ $value->good_image }}">
                            </a>
                        </td>
                        <td class="text-l">
                            {{ $value->good_name }}
                        </td>
                        <td class="text-l">{{ $value->good_details }}</td>
                        <td>
                            <span class="price">
                                {{ $value->good_recommend }}
                            </span>
                        </td>
                        <td class="td-status">
                            @if($value->good_tag == 0)
                                <span class="label label-success radius">默认</span>
                            @elseif($value->good_tag ==10)
                                <span class="label label-success radius">推荐</span>
                            @elseif($value->good_tag == 20)
                                <span class="label label-success radius">热销</span>
                            @elseif($value->good_tag == 30)
                                <span class="label label-success radius">最新推荐</span>
                            @endif
                        </td>
                        <td class="td-manage">
...
<script type="text/javascript">
    ...
    function product_del(obj, id) {
        layer.confirm('确认要删除吗？', function (index) {
            $.ajax({
                type: "post",
                data: {'table': 'good', 'file': 'good_id', 'where': id},
                dataType: "json",
                url: "{{URL('all/del')}}",
