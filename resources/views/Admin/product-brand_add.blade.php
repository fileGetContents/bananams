...
<div class="pd-20">
    <form action="{{$url}}" enctype="multipart/form-data" method="post" class="form form-horizontal"
        ...
        <div class="row cl">
            ...
            <div class="formControls col-10">
                <select name="travel_recommend" id="">
                    <option value="0">默认</option>
                    <option value="10">香蕉小发现</option>
                    <option value="20">周末不浪费</option>
                    <option value="30">香蕉精选</option>
                </select>
            </div>
        </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-10">
                <select name="travel_classify" id="">
                    <option value="10">小众旅行</option>
                    <option value="20">户外体检</option>
                    <option value="30">处境优选</option>
                </select>
            </div>
        </div>
        ...
        <div class="row cl">
            <label class="form-label col-2">安排时间:</label>
            <div class="formControls col-10">
                <table>
                    <tr>
                        <td>开始时间:</td>
                        ...
                        {{--<td>结束时间:</td>--}}
                        {{--<td><input name="over[]" type="datetime-local"/></td>--}}
                        <td>周几:</td>
                        <td>
                            <select name="week[]" id="">
                        ...
                        <td>剩余数量:</td>
                        <td><input class="input-text" type="text" name="info_num[]" value=""></td>
                    </tr>
                </table>
            </div>
            <a href="javascript:void(0)" id="add_time">添加</a>
        </div>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-10">
                <div class="uploader-thum-container">
                    ...
                    {{--<input name="image_host" value="" id="image_host"  hidden="hidden" type="text"/>--}}
        ...
        <div class="row cl">
            <div class="col-10 col-offset-2">
                <button id="submit" class="btn btn-primary radius" type="submit">
                    <i class="Hui-i confont">&#xe632;</i> 保存并提交审核
                </button>
...
<script type="text/javascript">
    $(function () {
        ...
        $("#add_time").click(function () {
            $(this).prev().children().eq(0).append('<tr><td>开始时间:</td><td><input name="start[]" type="datetime-local"/></td><td>周几:</td> <td><select name="week[]" id=""><option value="星期一">星期一</option><option value="星期二">星期二</option><option value="星期三">星期三</option><option value="星期四">星期四</option> <option value="星期五">星期五</option><option value="星期六">星期六</option><option value="星期日">星期日</option></select></td></tr><td>剩余数量:</td><td><input class="input-text" type="text" name="info_num[]" value=""></td>')
        });
                    ...
                    if (cur === 'error' || cur === 'invalid') {
                        ...
                        $.ajax({
                            ...
                            success: function (obj) {
                                $li.append('<input hidden="hidden" type="" name="image[]" value="' + obj.name + '"/>');
                            },
                            error: function () {
                            }
                        });
    ...
    $.ajax({
        ...
        success: function (obj) {
            ...
            for (var i = 0; i < obj.length; i++) {
                pro += '<option value="' + obj[i].ProvinceID + '">' + obj[i].ProvinceName + '</option>';
            }
            $("#pro").append(pro);
        },
        ...
        if ($(this).val() > 0) {
            $.ajax({
                ...
                success: function (obj) {
                    ...
                    for (var i = 0; i < obj.length; i++) {
                        city += '<option value="' + obj[i].CityID + '">' + obj[i].CityName + '</option>';
                    }
        ...
        if ($(this).val() > 0) {
            $.ajax({
                ...
                success: function (obj) {
                    ...
                    for (var i = 0; i < obj.length; i++) {
                        district += '<option value="' + obj[i].DistrictID + '">' + obj[i].DistrictName + '</option>';
                    }
                    $("#dis").empty().append(district);
                },
                error: function (error) {
