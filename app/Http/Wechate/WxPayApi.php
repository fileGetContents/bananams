...
class WxPayApi
{
    ...
    public function unifiedOrder($inputObj, $timeOut = 6)
    {
        ...
        if (!$inputObj->IsOut_trade_noSet()) {
            echo 1;
            //    new WxPayException("缺少统一支付接口必填参数out_trade_no!");
        } else if (!$inputObj->IsBodySet()) {
            echo 2;
            //   new WxPayException("缺少统一支付接口必填参数body!");
        } else if (!$inputObj->IsTotal_feeSet()) {
            echo 3;
            //   new WxPayException("缺少统一支付接口必填参数total_fee!");
        } else if (!$inputObj->IsTrade_typeSet()) {
            echo 4;
            //   new WxPayException("缺少统一支付接口必填参数trade_type!");
        }

        //关联参数
        if ($inputObj->GetTrade_type() == "JSAPI" && !$inputObj->IsOpenidSet()) {
            echo 5;
            //  new WxPayException("统一支付接口中，缺少必填参数openid！trade_type为JSAPI时，openid为必填参数！");
        }
        if ($inputObj->GetTrade_type() == "NATIVE" && !$inputObj->IsProduct_idSet()) {
            echo 6;
            //  new WxPayException("统一支付接口中，缺少必填参数product_id！trade_type为JSAPI时，product_id为必填参数！");
        }
